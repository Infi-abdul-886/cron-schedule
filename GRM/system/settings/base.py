...
INSTALLED_APPS = [
    ...
    # 'apps.cronjob',
    "apps.query_builder",   # ✅ use full dotted path
    'apps.workflow_engine.apps.WorkflowEngineConfig', # <-- Use this full path
    'apps.cronjob.apps.CronjobConfig', # Add your app's config class
    'apps.workflow_app',
    'apps.query',
]
...
NOTIFICATION_SETTINGS = {
    'HOURS_PAST': 24,  # e.g., send notification if not updated in 24 hours
    'STATUSES_TO_CHECK': [1,4,5,10,11,12,15],  # List of status IDs to monitor
}

CRONJOBS = [
    ('*/1 * * * *', 'apps.cronjob.cron.hello','>> /var/www/html/grouprm/GRM/cron.log 2>&1'),
    ('*/1 * * * *', 'apps.cronjob.views.adminNotificationView.run_stale_request_notification', '>> /var/www/html/grouprm/GRM/cron1.log 2>&1'),

    # ('*/5 * * * *', 'apps.cronjob.views.updatePNR.updatePNR.autoUpdatePNR'),
]
APSCHEDULER_DATETIME_FORMAT = "N j, Y, f:s a"  # Optional: to format datetime fields
...
# settings.py
NOTIFICATION_MODE = "php"   # "php" (restrictive) or "django" (comprehensive)

# settings.py

APPROVE_PAGE_SETTINGS = {
    'AUTOPILOT_EMAIL_ID': 'grouprm@infinitisoftware.net' 
}
STATICFILES_DIRS = [BASE_DIR / 'query/static']
